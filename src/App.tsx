...
const WhatsAppSticky = lazy(() => import("./components/WhatsAppSticky").then(m => ({ default: m.WhatsAppSticky })));
const DiscountPopup = lazy(() => import("./components/DiscountPopup").then(m => ({ default: m.DiscountPopup })));
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 selection:bg-brand-primary selection:text-slate-950 overflow-x-hidden pb-24 md:pb-0">
      ...
      <Suspense fallback={null}>
        <AnimatePresence>
          {!isHeroInView && <WhatsAppSticky key="whatsapp-sticky" />}
        </AnimatePresence>
        <DiscountPopup key="discount-popup" />
      </Suspense>
    </div>
  );
}
